...
## Constraints
Number of rows `r` and number of columns `c` satisfy `1 ≤ r,c ≤ 500`. So you should think about efficiency (a little).

### Note
Matrix saddle points are used in game theory to identify optimal strategies in two-person zero-sum games - see [here](https://themba.institute/operations-research/saddle-points/). There is a related saddle point concept for functions, which has even wider applications - see [here](https://en.wikipedia.org/wiki/Saddle_point).